...
Some of the key "themes" of this release are:
    ...
    # *Distributed AIR apps* (Better push/pull workflow, out of the box distributed configuration for Rails back-end, etc)
  
    # *schema_to_yaml integration* (helps with migrating existing Rails projects over to RestfulX)
...
Easier synchronization is one of the "key" themes in this release.

For starters, you can generate distribution ready apps, pretty much out of the box. Here's a few steps to get you going:
...
Your Rails app will be configured to use UUIDs for all models and references, and the generated controllers will
behave in a way that is more consistent with distributed RESTful behaviour (a-la CouchDB).

Additionally, you can perform both push and pull operations against a remote service provider.
...
Chaining Rx.filter(Rx.sort()) now actually works.
...
You can now send custom HTTP headers with Rx.http() and XMLHTTPServiceProvider/JSONHTTPServiceProvider. 
Just set Rx.customHTTPHeaders and off you go.

You can now use [HasMany(sort="foo:descending,caseInscensitive,numeric")] annotations in your RestfulX Flex models. It goes something along these lines:
{% highlight as3 %}
